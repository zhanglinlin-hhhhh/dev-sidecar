...
def json2graph(json_data,
    ...
    for key,value in json_data.items():
        ...
        for key2,value2 in value.items():
            nodes.append( Node(id="%s_%s"%(key,key2),
                            label=value2,
                            size=5,
                            shape="dot",
                            ) 
                        )
            edges.append( Edge(source="%s_%s"%(key,key2),
                            label=" ",
                            target=key,
                            ) 
                        )
        edges.append( Edge(source=key,
                        label=" ",
                        target="Topic",
                        ) 
                    )
...
with st.expander("See explanation"):
    nodes, edges, config = json2graph(json_data, topic)
